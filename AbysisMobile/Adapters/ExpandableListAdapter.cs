...
namespace AbysisMobile.Adapters
{
    class ExpandableListAdapter : BaseExpandableListAdapter
    {
        ...
        public override View GetChildView(int groupPosition, int childPosition, bool isLastChild, View convertView, ViewGroup parent)
        {
            ...
            ImageView img= (ImageView)convertView.FindViewById(Resource.Id.ivMeterImg);
            if(childText.StartsWith('1'))
            {
                img.SetImageResource(Resource.Drawable.light);
            }
            else if (childText.StartsWith('2'))
            {
                img.SetImageResource(Resource.Drawable.drop);
            }
            else if (childText.StartsWith('3'))
            {
                img.SetImageResource(Resource.Drawable.gas);
            }
            else if (childText.StartsWith('5'))
            {
                img.SetImageResource(Resource.Drawable.comminicate);
            }
